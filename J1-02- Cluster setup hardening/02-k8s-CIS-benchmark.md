...
You can perform CIS Benchmark over K8S cluster with several tools. Aqua Security provide two :
- Kube-bench https://github.com/aquasecurity/kube-bench (the old one)
- Trivy https://blog.aquasec.com/trivy-kubernetes-cis-benchmark-scanning (can now do CIS Benchmark among other thing)
...
### perform CIS scan with trivy

```sh
trivy k8s cluster --compliance=k8s-cis --report summary

# be patient
# if you kill the process en try again you will have the following error
# FATAL   get k8s artifacts with node info error: running node-collector job: jobs.batch "node-collector-7449988cd6" already exist

# check work in progress
kubectl get job -n trivy-temp
NAME                        COMPLETIONS   DURATION   AGE
node-collector-7449988cd6   0/1           2m7s       2m7s

# again, be patient
kubectl get job -n trivy-temp 
NAME                        COMPLETIONS   DURATION   AGE
node-collector-7449988cd6   0/1           4m44s      4m44s


# Error trivy k8s cluster --compliance=k8s-cis --report summary
2023-10-23T08:50:55.706Z        FATAL   get k8s artifacts with node info error: running node-collector job: runner received timeout

# Job still running
kubectl get job -n trivy-temp 
NAME                        COMPLETIONS   DURATION   AGE
node-collector-7449988cd6   0/1           6m53s      6m53s

kubectl get job -n trivy-temp 
NAME                        COMPLETIONS   DURATION   AGE
node-collector-7449988cd6   0/1           20m        20m
